define([], function() {
     ...
     *                            include in the grid.
     * @param {string} imageData[0].name - The name of the image.
    ...
    function Pig(imageData, options) {
        ...
        this.settings = {
            ...
            /**
             * Get the URL for an image with the given name & size.
             *
    ...
    Pig.prototype._computeLayout = function() {
        if (this.settings._computeLayout) return this.settings._computeLayout.call(this);
        // Constants
     ...
     *                                  include in the grid.
     * @param {string} singleImageData[0].name - The name of the image.
    ...
    function ProgressiveImage(singleImageData, index, pig) {
        ...
        this.name = singleImageData.name; // name
        // The Pig instance
        this.pig = pig;
    ...
    ProgressiveImage.prototype.load = function() {
        ...
        var originUrl = this.pig.settings.getImgUrl(this.singleImageData, this.pig.settings.originName),
            thumbUrl = this.pig.settings.getImgUrl(this.singleImageData, this.pig.settings.thumbnailName);
        this._updateStyles();
        this.pig.container.appendChild(this.getElement());
        if (!this.imageContainer) {
            this.imageContainer = document.createElement("a");
            this.imageContainer.href = originUrl;
            this.getElement().appendChild(this.imageContainer);
        }
        ...
        setTimeout(function() {
            ...
            if (!this.thumbnail) {
                ...
                this.imageContainer.appendChild(this.thumbnail);
            }

            // Show full image
            if (!this.fullImage) {
                this.fullImage = new Image();
                this.fullImage.src = originUrl;
                ...
                this.imageContainer.appendChild(this.fullImage);
